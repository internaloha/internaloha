...
const fileText = JSON.parse(rawData);
...
function getSentencesWithWord(word, textInput, date) {
  ...
  // Filters for sentences based on that word
...
// });
  ...
  let positionResults = '';
  let compensation = '';
  // Goes through every internship listing
  ...
  Logger.info('');
  fs.writeFileSync('../src/src/data/angellist.parsed.data.json', JSON.stringify(text, null, 4), 'utf-8');
}
