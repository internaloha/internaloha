  ...
  async generateListings() {
    ...
    while (await super.selectorExists(listingsTable)) {
      ...
      for (let i = 0; i < urls.length; i++) {
        ...
        const position = await super.getValues('h1[itemprop="title"]', 'innerText');
        const description = await super.getValues('div[id="jd-description"]', 'innerText');
        ...
        const listing = new Listing({
          url: urls[i], position: position[0], location, company: 'Apple', description:
            description[0]
        });
